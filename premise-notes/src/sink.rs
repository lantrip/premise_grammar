        ...
        if self.pretty {
            ...
            for b in beats {
                println!(
                    "- {}{}",
                    b.text,
                    if let Some(ref s) = b.section {
                        if let Some(ref scene) = s.scene {
                            format!(" (scene: {})", scene)
                        } else {
                            String::new()
                        }
...
impl NotesSink for JsonlDirSink {
    fn emit_beats(&mut self, beats: &[Beat]) -> std::io::Result<()> {
        ...
        crate::io::append_many_jsonl(&path, beats)
    }
    fn emit_facts(&mut self, facts: &[Fact]) -> std::io::Result<()> {
        let path = self.dir.join("facts.jsonl");
        crate::io::append_many_jsonl(&path, facts)
    }
    fn emit_timeline(&mut self, events: &[TimelineEvent]) -> std::io::Result<()> {
        let path = self.dir.join("timeline.jsonl");
        crate::io::append_many_jsonl(&path, events)
    }
}
...
impl DirSink {
    ...
    fn write_many<T: serde::Serialize>(
        base: &Path,
        sub: &str,
        id_fn: impl Fn(&T) -> &str,
        values: &[T],
