...
use crate::schema::{Beat, BeatSource, Fact, FactSource, FactType, Section, Timeline, TimelineEvent, TimelineOrder, TimelineOrderName, Provenance};
...
pub fn extract_beats_from_text(text: &str, opts: &TextExtractionOptions) -> Vec<Beat> {
    ...
    let file = opts.file_label.clone().unwrap_or_else(|| "stdin".to_string());
...
pub fn extract_beats_from_markdown(text: &str, opts: &TextExtractionOptions) -> Vec<Beat> {
    let file = opts.file_label.clone().unwrap_or_else(|| "stdin".to_string());
    let mut beats: Vec<Beat> = Vec::new();
    let mut current_section: Section = Section { act: None, scene: None, cel: None };
    let mut paragraph_lines: Vec<(usize, String)> = Vec::new();

    let flush_paragraph = |para: &mut Vec<(usize, String)>, beats: &mut Vec<Beat>, section: &Section| {
        if para.is_empty() { return; }
        let start_line = para.first().map(|(ln, _)| *ln).unwrap_or(1);
        let text = para.iter().map(|(_, s)| s.as_str()).collect::<Vec<_>>().join(" ").trim().to_string();
        if text.is_empty() { para.clear(); return; }
    ...
    for (i, raw) in text.lines().enumerate() {
        ...
        if let Some((level, title)) = parse_markdown_heading(line) {
            flush_paragraph(&mut paragraph_lines, &mut beats, &current_section);
            match level {
                1 => { current_section.act = Some(title.to_string()); current_section.scene = None; current_section.cel = None; }
                2 => { current_section.scene = Some(title.to_string()); current_section.cel = None; }
                3 => { current_section.cel = Some(title.to_string()); }
                _ => {}
            }
            continue;
        }
...
pub fn extract_beats_from_transcript(text: &str, opts: &TextExtractionOptions) -> Vec<Beat> {
    ...
    let timecode_re = Regex::new(r"^\d{2}:\d{2}:\d{2}[\.,]\d{3}\s+-->\s+\d{2}:\d{2}:\d{2}[\.,]\d{3}").unwrap();
    ...
    for (i, line) in text.lines().enumerate() {
        ...
        if lt.chars().all(|c| c.is_ascii_digit()) { continue; }
        if timecode_re.is_match(lt) { continue; }
