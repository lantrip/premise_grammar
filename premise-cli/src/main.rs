...
use premise_core::notes::entity_discovery::{
    discover_entities_from_narrative, extract_entities_with_aliases, merge_aliases_with_candidates,
};
use premise_core::notes::entity_discovery::{
    EntityCandidate as CoreEntityCandidate, EntityType as CoreEntityType,
};
    ...
    match cli.command {
        ...
        Commands::Analyze {
            ...
            if extract_notes {
                ...
                let timeline_path =
                    premise_core::notes::get_notes_dir(story_root).join("timeline.jsonl");
        ...
        Commands::Schema { r#type, out } => {
            ...
            match r#type.as_str() {
                ...
                "notes" => {
                    schemas.push((
                        "Beat".into(),
                        schemars::schema_for!(premise_core::notes::Beat),
                    ));
                    schemas.push((
                        "Fact".into(),
                        schemars::schema_for!(premise_core::notes::Fact),
                    ));
                    schemas.push((
                        "TimelineEvent".into(),
                        schemars::schema_for!(premise_core::notes::TimelineEvent),
                    ));
                    schemas.push((
                        "ConsistencyEntry".into(),
                        schemars::schema_for!(premise_core::notes::ConsistencyEntry),
                    ));
                    schemas.push((
                        "NotesIndex".into(),
                        schemars::schema_for!(premise_core::notes::NotesIndex),
                    ));
                    schemas.push((
                        "NotesMetadata".into(),
                        schemars::schema_for!(premise_core::notes::NotesMetadata),
                    ));
                }
                _ => {
                    schemas.push((
            ...
            match notes_cmd {
                NotesCommands::Init { path, title } => {
                    match premise_core::notes::initialize_notes(&path, title) {
                        ...
                        Err(e) => {
                            eprintln!(
                                "Error: Failed to initialize notes directory at {}: {}",
                                path.display(),
                                e
                            );
                            std::process::exit(1);
                ...
                NotesCommands::ExportBeats {
                    ...
                    let beats = match input {
                        InputMode::Prem => {
                            ...
                            let beats = premise_core::notes::extract_beats_from_tree(
                                &root, &content, file_str,
                            );
                            let alias_map =
                                extract_entities_with_aliases(&root, &content, file_str);
                        ...
                        InputMode::Plain | InputMode::Markdown => {
                            ...
                            let alias_map = if let Some(path) = aliases {
                                ...
                                let (map, _) =
                                    premise_notes::io::load_aliases_with(story_root, &[])
                                        .unwrap_or((
                                            std::collections::HashMap::new(),
                                            premise_notes::io::AliasMergeReport {
                                                added: 0,
                                                conflicts: vec![],
                                            },
                                        ));
                    ...
                    if stable_ids {
                        ...
                        for b in &mut beats {
                            ...
                            let id = premise_notes::io::stable_id(
                                "beat_",
                                &[&b.file, &line_str, &b.text],
                            );
                            b.id = id;
                        }
                        if dry_run {
                        ...
                        match sink {
                            Sink::Notes => {
                                ...
                                {
                                    eprintln!(
                                        "Error: Failed to initialize notes at {}: {}",
                                        story_root.display(),
                                        e
                                    );
                                    std::process::exit(1);
                                ...
                                if let Err(e) =
                                    premise_core::notes::append_beats(story_root, &beats)
                                {
                                ...
                                for b in &beats {
                                    let u =
                                        premise_core::notes::collect_uncertain_entities_from_text(
                                            &b.text,
                                        );
                                    if !u.is_empty() {
                                        total_uncertain += 1;
                            ...
                            Sink::JsonlDir => {
                                ...
                                let mut s = match premise_core::notes::JsonlDirSink::new(&dir) {
                                    ...
                                    Err(e) => {
                                        eprintln!(
                                            "Error: Failed to prepare output dir {}: {}",
                                            dir.display(),
                                            e
                                        );
                                        std::process::exit(1);
                ...
                NotesCommands::ExtractFacts {
                    ...
                    let facts = match input {
                        InputMode::Prem => {
                            ...
                            let root = tree.root_node();
                            let facts = premise_core::notes::extract_all_facts_from_tree(
                                &root, &content, file_str,
                            );
                        ...
                        InputMode::Plain | InputMode::Markdown => {
                            let story_root =
                                file.parent().unwrap_or_else(|| std::path::Path::new("."));
                            alias_map = if let Some(path) = aliases {
                                std::fs::read_to_string(&path)
                                    .ok()
                                    .and_then(|s| serde_json::from_str(&s).ok())
                                    .unwrap_or_default()
                            ...
                            let params = premise_notes::extract_pipeline::FactParams {
                                content: &content,
                                file_label: file_str,
                                aliases: Some(&alias_map),
                                min_importance,
                    ...
                    for f in &mut facts {
                        if f.importance.is_none() {
                            let base = if matches!(
                                f.fact_type,
                                premise_notes::schema::FactType::Event
                                    | premise_notes::schema::FactType::Relationship
                            ) {
                                0.8
                            } else {
                                0.4
                            ...
                            if score >= 0.5 {
                                f.importance = Some(premise_notes::schema::Importance {
                                    score,
                                    assessed_by: premise_notes::schema::ImportanceSource::Heuristic,
                                ...
                                if let Some(list) = &mut f.importance_assessments {
                                    ...
                                    f.importance_assessments =
                                        Some(vec![f.importance.clone().unwrap()]);
                                }
                            }
                        }
                    }

                    if let Some(threshold) = min_importance {
                        facts.retain(|f| {
                            f.importance
                                .as_ref()
                    ...
                    match sink {
                        Sink::Notes => {
                            ...
                            let unknowns = premise_core::notes::collect_unknown_fact_entities(
                                &facts, &canonical,
                            );
                            if !unknowns.is_empty() {
                                eprintln!(
                                    "Note: {} unknown entity reference(s) found in facts: {}",
                                    unknowns.len(),
                                    unknowns.join(", ")
                                );
                ...
                NotesCommands::ExtractTimeline {
                    ...
                    let mut timeline = match input {
                        ...
                        InputMode::Plain | InputMode::Markdown => {
                            premise_notes::extract_text::extract_timeline_from_text(
                                &content, file_str,
                            )
                        }
                    };
                    ...
                    if stable_ids {
                        for ev in &mut timeline {
                            let src = ev.source.first().cloned().unwrap_or_default();
                    ...
                    match sink {
                        Sink::Notes => {
                            ...
                            premise_core::notes::initialize_notes(story_root, None)
                                .expect("failed to initialize notes");
                            let timeline_path = premise_core::notes::get_notes_dir(story_root)
                                .join("timeline.jsonl");
                            premise_core::notes::append_many_jsonl(&timeline_path, &timeline)
                                .expect("failed to append timeline");
                            ...
                            let mut total_uncertain: usize = 0;
                            for ev in &timeline {
                                let u = premise_core::notes::collect_uncertain_entities_from_text(
                                    &ev.event,
                                );
                                if !u.is_empty() {
                                    total_uncertain += 1;
                            ...
                            if total_uncertain > 0 {
                                eprintln!("Note: {} timeline event(s) contain uncertain entities (e.g., {{?Name}}). Consider running 'premise notes discover-entities'.", total_uncertain);
                            }
                            println!(
                                "Extracted {} timeline events from {}",
                                timeline.len(),
                                file.display()
                            );
                        }
                ...
                NotesCommands::Normalize { path, dry_run } => {
                    ...
                    for fpath in files.iter() {
                        ...
                        let content = match fs::read_to_string(&full_path) {
                            ...
                            Err(e) => {
                                eprintln!(
                                    "Warning: Failed to read source file {}: {}",
                                    full_path.display(),
                                    e
                                );
                                continue;
                        ...
                        let root = tree.root_node();
                        let alias_map = extract_entities_with_aliases(
                            &root,
                            &content,
                    ...
                    for m in per_file.values() {
                        for c in &m.canonical {
                            global_canonical.insert(c.clone());
                        }
                    ...
                    for mut b in beats {
                        let (text, ents) = if let Some(m) = per_file.get(&b.file) {
                            premise_core::notes::normalize_beat_text(
                                &b.text,
                                &m.canonical,
                                &m.reverse,
                            )
                        ...
                        if !premise_core::notes::collect_uncertain_entities_from_text(&text)
                            .is_empty()
                        {
                    ...
                    let normalized_facts =
                        premise_core::notes::normalize_facts(facts, &global_reverse);
                    ...
                    if dry_run {
                        ...
                        if let Err(e) =
                            premise_core::notes::write_jsonl(&beats_path, &normalized_beats)
                        {
                            eprintln!("Error: Failed to write normalized beats: {}", e);
                            std::process::exit(1);
                        }
                        if let Err(e) =
                            premise_core::notes::write_jsonl(&facts_path, &normalized_facts)
                        {
                    ...
                    if beats_uncertain_count > 0 {
                        eprintln!("Note: {} beat(s) still contain uncertain entities (e.g., {{?Name}}). Consider running 'premise notes discover-entities'.", beats_uncertain_count);
                    }
                ...
                NotesCommands::RebuildIndex { path } => {
                    let index = match premise_core::notes::rebuild_index(&path) {
                        ...
                        Err(e) => {
                            eprintln!(
                                "Error: Failed to rebuild index at {}: {}",
                                path.display(),
                                e
                            );
                            std::process::exit(1);
                ...
                NotesCommands::Query { entity, path } => {
                    ...
                    if let Some(record_ids) = index.entity_index.get(&entity) {
                        let beats =
                            premise_core::notes::read_beats(&path).expect("failed to read beats");
                        let facts =
                            premise_core::notes::read_facts(&path).expect("failed to read facts");
                ...
                NotesCommands::SummarizeUncertain { path } => {
                    match premise_notes::orchestrate::summarize_uncertainties(&path) {
                        Ok((unknown_entities, files)) => match cli.globals.format {
                            Format::Json => {
                                let out = serde_json::json!({ "unknown_entities": unknown_entities, "files": files });
                                println!("{}", serde_json::to_string_pretty(&out).unwrap());
                            }
                            Format::Pretty => {
                ...
                NotesCommands::MergeAliases {
                    aliases,
                    path,
                    dry_run,
                } => {
                    let incoming: std::collections::HashMap<String, Vec<String>> =
                    ...
                    let mut base = match premise_notes::io::read_alias_map(&path) {
                        Ok(m) => m,
                        Err(e) => {
                            eprintln!("Error: Failed to read existing aliases: {}", e);
                            std::process::exit(1);
                        }
                    };
                    let report = premise_notes::io::merge_alias_maps(&mut base, &incoming);
                    if dry_run {
                        println!("{}", serde_json::to_string_pretty(&serde_json::json!({ "added": report.added, "conflicts": report.conflicts })).unwrap());
                    } else if let Err(e) = premise_notes::io::write_alias_map(&path, &base) {
                        eprintln!("Error: Failed to write aliases: {}", e);
                        std::process::exit(1);
                    } else {
                        println!(
                            "Merged aliases. Added: {}. Conflicts: {}",
                            report.added,
                            report.conflicts.len()
                        );
                    }
                }
                NotesCommands::ApplyAliasDelta {
                    delta,
                    path,
                    dry_run,
                } => {
                    let incoming: std::collections::HashMap<String, Vec<String>> =
                        match std::fs::read_to_string(&delta)
                            .ok()
                            .and_then(|s| serde_json::from_str(&s).ok())
                        {
                            Some(m) => m,
                            None => {
                                eprintln!("Error: Failed to read delta file {}", delta.display());
                                std::process::exit(1);
                            }
                        };
                    let mut base = match premise_notes::io::read_alias_map(&path) {
                    ...
                    if dry_run {
                        ...
                        println!(
                            "Applied alias delta. Added: {}. Conflicts: {}",
                            report.added,
                            report.conflicts.len()
                        );
                    }
                }
                NotesCommands::SetImportance {
                    id,
                    score,
                    source,
                    method,
                    ...
                    let mut beats = match premise_core::notes::read_beats(&path) {
                        Ok(b) => b,
                        Err(e) => {
                            eprintln!("Error: {}", e);
                            std::process::exit(1);
                    ...
                    let mut facts = match premise_core::notes::read_facts(&path) {
                        Ok(f) => f,
                        Err(e) => {
                            eprintln!("Error: {}", e);
                            std::process::exit(1);
                    ...
                    let src = match source {
                        ImportanceOrigin::User => premise_notes::schema::ImportanceSource::User,
                        ImportanceOrigin::Ai => premise_notes::schema::ImportanceSource::Ai,
                        ImportanceOrigin::Heuristic => {
                            premise_notes::schema::ImportanceSource::Heuristic
                        }
                    };
                    let imp = premise_notes::schema::Importance {
                        score,
                        assessed_by: src,
                        method,
                        updated: chrono::Utc::now().to_rfc3339(),
                    ...
                    for b in &mut beats {
                        if b.id == id {
                            if let Some(list) = &mut b.importance_assessments {
                                ...
                                b.importance_assessments = Some(vec![imp.clone()]);
                            }

                            if let Some(list) = &b.importance_assessments {
                                b.importance =
                                    premise_notes::importance::choose_authoritative(list);
                            }
                            updated = true;
                            break;
                        }
                        ...
                        for f in &mut facts {
                            if f.id == id {
                                if let Some(list) = &mut f.importance_assessments {
                                    ...
                                    f.importance_assessments = Some(vec![imp.clone()]);
                                }

                                if let Some(list) = &f.importance_assessments {
                                    f.importance =
                                        premise_notes::importance::choose_authoritative(list);
                                }
                                updated = true;
                    ...
                    if !updated {
                        eprintln!("Error: Record id '{}' not found in beats or facts", id);
                        std::process::exit(1);
                    }
                    ...
                    let beats_path = notes_dir.join("beats.jsonl");
                    let facts_path = notes_dir.join("facts.jsonl");
                    if let Err(e) = premise_core::notes::write_jsonl(&beats_path, &beats) {
                        eprintln!("Error writing beats: {}", e);
                        std::process::exit(1);
                    }
                    if let Err(e) = premise_core::notes::write_jsonl(&facts_path, &facts) {
                        eprintln!("Error writing facts: {}", e);
                        std::process::exit(1);
                    }
                    println!("Set importance for '{}' successfully.", id);
                }
                NotesCommands::DiscoverEntities {
                    file,
                    input,
                    stdin,
                    aliases,
                    ...
                    match input {
                        InputMode::Prem => {
                            ...
                            let root = tree.root_node();
                            alias_map = extract_entities_with_aliases(&root, &content, file_str);
                            let known_entities: HashSet<String> =
                                alias_map.keys().cloned().collect();
                            candidates = discover_entities_from_narrative(
                                &root,
                                &content,
                                file_str,
                                &known_entities,
                        ...
                        InputMode::Plain | InputMode::Markdown => {
                            if let Some(path) = aliases {
                                alias_map = std::fs::read_to_string(&path)
                                    .ok()
                                    .and_then(|s| serde_json::from_str(&s).ok())
                                    .unwrap_or_default();
                            } else {
                            ...
                            let known_entities: HashSet<String> =
                                alias_map.keys().cloned().collect();
                            candidates = premise_notes::discovery::discover_entities_from_text(
                                &content,
                                &known_entities,
                            )
                            .into_iter()
                            .map(|c| CoreEntityCandidate {
                                canonical_name: c.canonical_name,
                                entity_type: match c.entity_type.as_str() {
                                    "location" => CoreEntityType::Location,
                                    "object" => CoreEntityType::Object,
                                    "concept" => CoreEntityType::Concept,
                                    _ => CoreEntityType::Character,
                                },
                                aliases: c.aliases,
                    ...
                    if let Ok(existing_beats) = premise_core::notes::read_beats(story_root) {
                        for b in existing_beats {
                            let mut u =
                                premise_core::notes::collect_uncertain_entities_from_text(&b.text);
                    ...
                    let delta = {
                        let canon_candidates: Vec<premise_notes::discovery::EntityCandidate> =
                            candidates
                                .iter()
                                .map(|c| premise_notes::discovery::EntityCandidate {
                                    canonical_name: c.canonical_name.clone(),
                                    entity_type: match c.entity_type {
                                        CoreEntityType::Location => {
                                            premise_notes::discovery::EntityType::Location
                                        }
                                        CoreEntityType::Object => {
                                            premise_notes::discovery::EntityType::Object
                                        }
                                        CoreEntityType::Concept => {
                                            premise_notes::discovery::EntityType::Concept
                                        }
                                        _ => premise_notes::discovery::EntityType::Character,
                                    },
                                ...
                                .collect();
                        premise_notes::orchestrate::propose_alias_updates_api(
                            &canon_candidates,
                            &alias_map,
                        )
                    };
                    ...
                    match cli.globals.format {
                        ...
                        Format::Pretty => {
                            ...
                            if !delta.additions.is_empty() {
                                ...
                                for (canon, aliases) in delta.additions.iter() {
                                    println!("  {} += [{}]", canon, aliases.join(", "));
                                }
                            }
                        }
                    }
                ...
                NotesCommands::ReviewExtraction {
                    proposal,
                    path,
                    strategy,
                    min_confidence,
                    ...
                    let proposal_str = if proposal == "-" {
                        ...
                        match fs::read_to_string(&proposal) {
                            Ok(s) => s,
                            Err(e) => {
                                eprintln!("Error: Failed to read proposal {}: {}", proposal, e);
                                std::process::exit(1);
                            }
                        }
                    };
                    ...
                    struct Proposal {
                        #[serde(default)]
                        beats: Vec<premise_notes::schema::Beat>,
                        #[serde(default)]
                        facts: Vec<premise_notes::schema::Fact>,
                        #[serde(default)]
                        alias_delta: std::collections::HashMap<String, Vec<String>>,
                    }
                    ...
                    let canonical: std::collections::HashSet<String> =
                        premise_notes::io::read_alias_map(&path)
                            .unwrap_or_default()
                            .keys()
                            .cloned()
                            .collect();
                    ...
                    let actions = Actions {
                        accept_beats: &accepted_beats,
                        accept_facts: &accepted_facts,
                        apply_alias_delta: &proposal.alias_delta,
                        summary,
                    };

                    if let Some(out) = out_actions.as_ref() {
                        if let Err(e) =
                            std::fs::write(out, serde_json::to_string_pretty(&actions).unwrap())
                        {
                            eprintln!("Error: Failed to write actions {}: {}", out.display(), e);
                            std::process::exit(1);
                        }
                    }
                    ...
                    if apply {
                        ...
                        if let Err(e) =
                            premise_core::notes::append_many_jsonl(&beats_path, &accepted_beats)
                        {
                            eprintln!("Error: {}", e);
                            std::process::exit(1);
                        }
                        if let Err(e) =
                            premise_core::notes::append_many_jsonl(&facts_path, &accepted_facts)
                        {
                            eprintln!("Error: {}", e);
                            std::process::exit(1);
                        }
                        if !proposal.alias_delta.is_empty() {
                            let mut base =
                                premise_notes::io::read_alias_map(&path).unwrap_or_default();
                            let _ = premise_notes::io::merge_alias_maps(
                                &mut base,
                                &proposal.alias_delta,
                            );
                            if let Err(e) = premise_notes::io::write_alias_map(&path, &base) {
                                eprintln!("Error: {}", e);
                                std::process::exit(1);
                            }
                        }
                ...
                NotesCommands::ApplyActions {
                    actions,
                    path,
                    dry_run,
                } => {
                    let data = match std::fs::read_to_string(&actions) {
                        Ok(s) => s,
                        Err(e) => {
                            eprintln!("Error: Failed to read actions {}: {}", actions.display(), e);
                            std::process::exit(1);
                        }
                    };
                    #[derive(serde::Deserialize)]
                    struct ActionsIn {
                        #[serde(default)]
                        accept_beats: Vec<premise_notes::schema::Beat>,
                        #[serde(default)]
                        accept_facts: Vec<premise_notes::schema::Fact>,
                        #[serde(default)]
                        apply_alias_delta: std::collections::HashMap<String, Vec<String>>,
                    }
                    let actions: ActionsIn = match serde_json::from_str(&data) {
                        Ok(v) => v,
                        Err(e) => {
                    ...
                    premise_core::notes::initialize_notes(&path, None).ok();
                    if let Err(e) =
                        premise_core::notes::append_many_jsonl(&beats_path, &actions.accept_beats)
                    {
                        eprintln!("Error: {}", e);
                        std::process::exit(1);
                    }
                    if let Err(e) =
                        premise_core::notes::append_many_jsonl(&facts_path, &actions.accept_facts)
                    {
                        eprintln!("Error: {}", e);
                        std::process::exit(1);
                    }
        ...
        Commands::Adapters(adapters_cmd) => match adapters_cmd {
                ...
                if let Some(adapter) = registry.find_by_id(&id) {
                    match cli.globals.format {
                        ...
                        Format::Pretty => {
                            ...
                            println!(
                                "  Generates artifacts: {}",
                                adapter.capabilities.generates_artifacts
                            );
                            println!(
                                "  Uses notes context: {}",
                                adapter.capabilities.uses_notes_context
                            );
                            println!(
                                "  Contributes facts: {}",
                                adapter.capabilities.contributes_facts
                            );
                            println!(
                                "  Output formats: {}",
                                adapter.capabilities.output_formats.join(", ")
                            );
                            println!(
                                "  Entity types: {}",
                                adapter.capabilities.entity_types.join(", ")
                            );
