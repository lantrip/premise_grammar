...
function normalizeFileUri(input: vscode.Uri | string): vscode.Uri {
  try {
...
function coerceToFileUri(input: any): vscode.Uri | undefined {
  try {
    if (!input) return undefined;
    if (input instanceof vscode.Uri) return normalizeFileUri(input);
    if (typeof input === "string") return normalizeFileUri(input);
    // VSCode UriComponents shape
    if (typeof input === "object" && typeof input.scheme === "string") {
      try {
        const u = vscode.Uri.from(input as any);
...
export async function activate(context: vscode.ExtensionContext) {
  console.log(
    "🚀 Premise extension activate() called - timestamp:",
    Date.now()
  );
  ...
  try {
    ...
    if (serverPath) {
      ...
      await registerCommandOnce(
        ...
        async () => {
          ...
          try {
            ...
            if (pick) {
              const u = normalizeFileUri(pick.loc.uri as any);
      ...
      await registerCommandOnce(context, "premise.generateBeats", async () => {
        console.log(
          "🔥 premise.generateBeats command started - timestamp:",
          Date.now()
        );
        const editor = vscode.window.activeTextEditor;
        if (!editor) return;
        ...
        let singleSectionRegion: { start: number; end: number } | undefined;
        if (scope === "current-section") {
          ...
          if (currentSection) {
            ...
            try {
              const s = correctedSection as any;
            ...
            console.log(
              "🎯 Section text to process:",
              textToProcess.substring(0, 200) + "..."
            );
          } else {
            ...
            try {
              const isHeader = (lineText: string): number | undefined => {
              ...
              let headerLine = -1;
              let headerLevel: number | undefined = undefined;
              ...
              if (headerLevel && headerLevel >= 3) synthetic.cel = "";
              else if (headerLevel && headerLevel === 2) synthetic.scene = "";
              else synthetic.act = "";
              ...
              vscode.window.showWarningMessage(
                "No section found at cursor position"
              );
              return;
            }
          }
        ...
        try {
          ...
          const rpm = Math.max(
            1,
            rateCfg.get<number>("rateLimitPerMinute", 60)
          );
          ...
          if (scope === "current-section" && singleSectionRegion) {
            const previewInserts = vscode.workspace
              ...
              .get<boolean>("previewInserts", false);
            const neighborInfo = (() => {
              try {
                const headers = getSectionRegionsForStructure(
                  structureToProcess,
                  doc
                );
                const idx = headers.findIndex(
                  (h) =>
                    h.start === singleSectionRegion!.start &&
                    h.end === singleSectionRegion!.end
                );
                const prev = idx > 0 ? headers[idx - 1] : undefined;
                const next =
                  idx >= 0 && idx + 1 < headers.length
                    ? headers[idx + 1]
                    : undefined;
                const fmt = (x?: {
                  kind: "act" | "scene" | "cel";
                  title?: string;
                }) => (x ? `${x.kind}${x.title ? ` — ${x.title}` : ""}` : "");
                const prevStr = fmt(prev);
                const nextStr = fmt(next);
                return prevStr || nextStr
                  ? `\nNeighbors: ${prevStr ? `Prev: ${prevStr}` : ""}${
                      prevStr && nextStr ? "; " : ""
            ...
            const sectionText = getDocumentTextRange(
              doc,
              singleSectionRegion.start,
              singleSectionRegion.end
            );
            const userSection = `File: ${
              doc.uri.fsPath
            ...
            if (beats.length > 0) {
              ...
              if (previewInserts) {
                const choice = await vscode.window.showInformationMessage(
                  `Insert ${beats.length} beat(s) into ${path.basename(
                    doc.uri.fsPath
                  )}?`,
                  { modal: true, detail: beats.join("\n") },
                  "Apply",
                  "Cancel"
                );
                if (choice !== "Apply") return;
              }
              await insertBeatsAnchored(
                editor,
                beats,
                structureToProcess,
                [
          ...
          const targets: {
            ...
            text: string;
            structure: any;
            changed?: { startLine: number; endLine: number }[];
          }[] = [];
          if (scope === "current-section") {
            ...
            if (uris.length === 0) {
              vscode.window.showInformationMessage(
                "No files to process in selected scope."
              );
              return;
            }
            const allStruct = await collectStructureForUris(
              uris.map((u) => normalizeFileUri(u).toString())
            );
            for (const u of uris) {
              ...
              const per = {
                sections: (allStruct?.sections || []).filter((s: any) => {
                  ...
                  if (s.uri) {
                    try {
                      ...
                      if (suri.toString() === nu.toString()) return true;
                    } catch {}
                  }
                  ...
                  if (s.file) {
                    try {
                      ...
                      if (sf.toString() === nu.toString()) return true;
                    } catch {}
                  }
                  return false;
                }),
          ...
          await vscode.window.withProgress(
            ...
            async (progress, token) => {
              ...
              for (const t of targets) {
                const d =
                  t.uri.toString() === doc.uri.toString()
                    ? doc
                ...
                let filtered =
                  t.changed && t.changed.length > 0
                    ? regions.filter((r) =>
                        t.changed!.some(
                          (cr) =>
                            !(r.end < cr.startLine || r.start > cr.endLine)
                        )
                      )
                    : regions;
                if (
                  ...
                  filtered = [{ start: 0, end: d.lineCount - 1 } as any];
                }
                work.push({
                  uri: t.uri,
              ...
              await runSequential(
                work,
                async ({ uri, region, structure: perStructure }) => {
                  if (token.isCancellationRequested) return;
                  const d =
                    uri.toString() === doc.uri.toString()
                      ? doc
                  ...
                  const neighborInfo = (() => {
                    try {
                      ...
                      const idx = headers.findIndex(
                        (h) => h.start === region.start && h.end === region.end
                      );
                      const prev = idx > 0 ? headers[idx - 1] : undefined;
                      const next =
                        idx >= 0 && idx + 1 < headers.length
                  ...
                  if (beats.length > 0) {
                    ...
                    if (previewInserts && !applyAllConfirmed) {
                      ...
                      if (choice === "Cancel") {
                        ...
                        processed += 1;
                        progress.report({
                          message: `Processed ${processed}/${work.reduce(
                            (a, w) => a + w.regions.length,
                            0
                          )} sections`,
                        });
                        return;
          ...
          if (totalBeats === 0 && totalEntities === 0) {
            vscode.window.showInformationMessage(
              "No beats or entity updates generated."
            );
          } else {
            vscode.window.setStatusBarMessage(
              `Inserted ${totalBeats} beat(s); updated ${totalEntities} entit(y/ies).`,
              4000
            );
          }
        } catch (err) {
          console.log(
            "🔥 premise.generateBeats command failed - timestamp:",
            Date.now()
          );
          vscode.window.showErrorMessage(
            `Generate beats failed: ${String(err)}`
          );
        }
        console.log(
          "🔥 premise.generateBeats command finished - timestamp:",
          Date.now()
        );
      });
      ...
      await registerCommandOnce(
        context,
        "premise.updateEntityDescriptions",
        async () => {
          const editor = vscode.window.activeTextEditor;
          if (!editor) return;
          const doc = editor.document;
          ...
          const chunkDefault = cfg2.get<string>("ai.chunkingMode", "section");
          ...
          const chunkingMode =
            (chunkPick?.val as "section" | "file") || chunkDefault;
          ...
          for (const t of targets) {
            ...
            if (chunkingMode === "file") {
              ...
              if (updates.length > 0) {
                totalEntities += await applyEntityDescriptionUpdatesInDocument(
                  d,
                  updates
                );
              }
              ...
              const filteredRegions =
                t.changed && t.changed.length > 0
                  ? regions.filter((r) =>
                      t.changed!.some(
                        (cr) => !(r.end < cr.startLine || r.start > cr.endLine)
                      )
                    )
                  : regions;
          ...
          if (totalEntities === 0) {
            vscode.window.showInformationMessage(
              "No entity updates suggested."
            );
          } else {
            vscode.window.setStatusBarMessage(
              `Updated ${totalEntities} entit(y/ies).`,
              3000
            );
          }
        }
...
function findCurrentSection(
  ...
  let lastSectionBefore: any = undefined;
    ...
    const extendedSection = {
      ...
      range: {
        ...lastSectionBefore.range,
          ...
          line: cursorLine + 10, // Include some lines after cursor for context
          character: 0,
        },
      },
    };
    console.log(
      "📝 Extended section range to include content:",
      extendedSection
    );
...
function getSectionText(doc: vscode.TextDocument, section: any): string {
...
async function getUncommittedChangedRangesForFile(
  absPath: string
  ...
  try {
    ...
    const out = cp.execSync(
      `git -C ${JSON.stringify(cwd)} diff -U0 -M -- ${JSON.stringify(rel)}`,
      { encoding: "utf8" }
    );
...
async function getStoryRootPathForUri(
  uri: vscode.Uri
): Promise<string | undefined> {
  if (!client) return undefined;
  try {
    console.log("🐛 getStoryRootPathForUri - uri:", uri.toString());
    const res = await client.sendRequest<any>("workspace/executeCommand", {
...
async function listPremFilesUnderRoot(rootPath: string): Promise<vscode.Uri[]> {
  const rootUri = vscode.Uri.file(rootPath);
  const pattern = new vscode.RelativePattern(rootUri, "**/*.prem");
  return vscode.workspace.findFiles(pattern);
}

function resolveGitTopLevel(startPath: string): string | undefined {
  try {
    const out = cp.execSync(
      `git -C ${JSON.stringify(startPath)} rev-parse --show-toplevel`,
      { encoding: "utf8" }
    );
    const top = out.trim();
...
async function listUncommittedPremFilesUnderRoot(
  rootPath: string
): Promise<vscode.Uri[]> {
  try {
    ...
    const out = cp.execSync(
      `git -C ${JSON.stringify(topLevel)} diff --name-only --diff-filter=ACMR`,
      { encoding: "utf8" }
    );
...
export async function insertBeatsAnchoredIntoDocument(
  ...
  for (const s of secs) {
    ...
    if (typeof start === "number") {
      if (typeof s.cel === "string") headers.push({ kind: "cel", start });
      else if (typeof s.scene === "string")
        headers.push({ kind: "scene", start });
      else if (typeof s.act === "string") headers.push({ kind: "act", start });
    }
  }
  headers.sort((a, b) => a.start - b.start);
  const pickFrom = (kinds: ("cel" | "scene" | "act")[]) => {
    const list = headers.filter((h) => kinds.includes(h.kind));
    if (list.length === 0)
      return undefined as { kind: any; start: number } | undefined;
    if (!changedRanges || changedRanges.length === 0)
      return list[list.length - 1];
    const inChanged = list.filter((h) =>
      changedRanges!.some((r) => h.start >= r.startLine && h.start <= r.endLine)
    );
    if (inChanged.length > 0) return inChanged[inChanged.length - 1];
    const maxChanged = Math.max(
      ...changedRanges.map((r) => Math.max(r.startLine, r.endLine))
    );
  ...
  const next =
    idx >= 0 && idx + 1 < headersSorted.length
      ? headersSorted[idx + 1]
      : undefined;
...
function getDocumentTextRange(
  ...
  const endPos = new vscode.Position(
    ...
    doc.lineAt(Math.min(doc.lineCount - 1, endLine)).range.end.character
  );
  return doc.getText(new vscode.Range(startPos, endPos));
}

async function applyEntityDescriptionUpdatesInDocument(
  doc: vscode.TextDocument,
  ...
  for (const u of updates) map.set(u.name, u.description);
...
async function listEntityNamesForUri(uri: vscode.Uri): Promise<string[]> {
  if (!client) return [];
  try {
    const res = await client.sendRequest<any>("workspace/executeCommand", {
...
async function collectStructureForUris(uris: string[]): Promise<any> {
...
function extractBeats(content: string): string[] {
  try {
...
function extractEntityTokens(beat: string): string[] {
  const out: string[] = [];
  const re = /\{([^}]+)\}/g;
  let m: RegExpExecArray | null;
...
function getBeatInsertPosition(): "append" | "prepend" {
  const cfg = vscode.workspace.getConfiguration("premise.ai");
  const pos = cfg.get<string>("beatInsertPosition", "append");
  return pos === "prepend" ? "prepend" : "append";
}

async function insertBeatsAtEnd(editor: vscode.TextEditor, beats: string[]) {
  const e = editor;
...
export async function insertBeatsAnchored(
  editor: vscode.TextEditor,
  beats: string[],
  structure: any,
  changedRanges?: { startLine: number; endLine: number }[],
  ...
  const next =
    idx >= 0 && idx + 1 < headers.length ? headers[idx + 1] : undefined;
  const regionStart = anchor.start + 1;
  const regionEnd = next
    ? Math.max(next.start - 1, regionStart)
    : doc.lineCount - 1;
