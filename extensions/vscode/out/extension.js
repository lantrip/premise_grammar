 ...
 */
async function execPremiseCli(args, cwd) {
 ...
 */
async function initializeNotes(storyRoot, title) {
 ...
 * Get notes status
 */
async function getNotesStatus(storyRoot) {
    try {
 ...
 * Rebuild notes index
 */
async function rebuildIndex(storyRoot) {
    await execPremiseCli(["notes", "rebuild-index"], storyRoot);
}
/**
 * Export beats from a file
 */
async function exportBeats(filePath, storyRoot) {
 ...
 */
async function extractFactsStructural(filePath, storyRoot) {
    const relativePath = path.relative(storyRoot, filePath);
 ...
 */
async function appendFacts(storyRoot, facts) {
        ...
        if (serverPath) {
            ...
            await registerCommandOnce(context, "premise.extractFacts", async () => {
                ...
                if (!notesStatus.initialized) {
                    const choice = await vscode.window.showInformationMessage("Notes system not initialized. Initialize now?", "Initialize", "Cancel");
                ...
                const user = `File: ${doc.uri.fsPath}\\nEntities: ${JSON.stringify(entityNames)}\\nStructure: ${JSON.stringify(structure)}${extra}\\n---\\n${textToProcess}\\n---\\n${schema}`;
                try {
                    const content = await openRouterChat({
                        endpoint,
                        apiKey,
                    ...
                    vscode.window.showInformationMessage(`Extracted ${extractedFacts.length} fact(s) to .premise-notes/facts.jsonl`);
                }
                catch (err) {
                    vscode.window.showErrorMessage(`Extract facts failed: ${String(err)}`);
                }
            });
            ...
            await registerCommandOnce(context, "premise.exportBeats", async () => {
                ...
                try {
                    ...
                    await rebuildIndex(storyRoot);
                    vscode.window.showInformationMessage("Beats exported to .premise-notes/beats.jsonl");
                }
                catch (err) {
...
function getFactExtractionSystemPrompt(categories) {
...
function getFactExtractionSchemaPrompt(categories) {
    return `Schema: {
  "facts": [
    {
...
function extractFacts(content, sourceFile, storyRoot) {
    try {
        const parsed = JSON.parse(content);
        ...
        return parsed.facts
            ...
            const fact = {
                ...
                entities: Array.isArray(f.entities)
                    ? f.entities.map((e) => String(e).trim()).filter(Boolean)
                    : undefined,
                category: f.category?.trim(),
                fact: String(f.fact || "").trim(),
                evidence: Array.isArray(f.evidence)
                    ? f.evidence.map((e) => String(e).trim()).filter(Boolean)
            ...
            if (!fact.status)
                delete fact.status;
            if (!fact.timeline)
                delete fact.timeline;
            return fact;
        })
            .filter((f) => f.fact && f.evidence.length > 0);
    }
    catch (err) {
